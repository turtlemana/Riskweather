...
const Top = ({EXP_VaRNTS,EXP_CVaRNTS,EXP_VaRNTS_95,EXP_CVaRNTS_95,exchg,cat,forcast, krName, riskDescriptionEn, riskDescriptionKr, name,ticker,varGauss,tailRisk,usdPrice,riskLevel,koreanRiskLevel, maxLoss,price,currency,priceChange,date,weather,weatherExplain, koreanWeatherExplain,tailRisk95, varGauss95, cVaRNTS95 }: Props) => {
    ...
    <main className="max-w-1320 min-w-[1024px] mx-auto p-8 rounded-20 bg-white">
      <header className="flex">
        <section className="flex-1">
          ...
          <Image loading="lazy" unoptimized   width={50} height={50} quality={100} onError={(event)=>handleImageError(event,exchg)}  src={`/images/logos/${ticker}.png`} alt="" className="h-[60px] w-[60px] mr-3 "/>           
       <div className="mr-7">
              <h5 className="font-bold text-[28px] h-[32px] mb-1">{router.locale=="ko" ? krName : name}</h5>
              <p className="text-gray-500">{ticker}</p>
            </div>
            <div
            data-tooltip-id="riskLevel" data-tooltip-content={router.locale=="ko" ? riskDescriptionKr : riskDescriptionEn}
              className={`py-[6px] px-4 rounded-20 text-center ${COLORS[riskLevel]}`}
            >
              <h6 className="h-5">{router.locale=="ko" ? koreanRiskLevel : riskLevel}</h6>
            </div>
          </article>
          <article className="flex items-center gap-6">
            <p className=" text-gray-500">{t('headerPrice')}</p>
            <h6 className="text-4xl">{price?.toLocaleString('en-us',{minimumFractionDigits: 0, maximumFractionDigits: 4})}{" "}{cat=="Index" ? "" : currency}</h6>
            <p className={`${priceChange > 0 ? "text-[#3CB043]" : "text-[#DF1525]"}`}>{priceChange?.toFixed(2)}%</p>
            <div className="w-px h-[20px] bg-gray-300 mx-1 mb-1" />
            <p className="text-gray-500">{t('headerLoss')}</p>
            <h6 className="text-4xl text-[#DF1525]">-{EXP_CVaRNTS?.toFixed(2)}%</h6>
          </article>
        </section>
        <section>
          <article className="flex items-center gap-5 mb-4 relative">
            ...
            {isActiveWeeklyModal && (
              //@ts-ignore
              <div ref={modalRef} className="z-30  absolute p-6 bg-white shadow-[0_0_12px_0_rgba(121,120,132,0.15)] top-8 right-0 rounded-20 border border-gray-100">
                <h6 className="mb-7">{t('headerForecast')}</h6>
                {forcastData?.map(({DATE_PlUS, WTHR_ENG_NM,WTHR_KR_DL,WTHR_ENG_DL }:Forcast, i:number) => (
                  <div className="flex items-center w-[221px] mb-3" key={i}>
                    ...
                    <Image src={`/images/weather/${WTHR_ENG_NM}.svg`} onError={handleWeatherImageError} width={50} height={50} alt="" quality={100} className="w-8 h-8" />
                    <h6 className="text-xs">{router.locale=="ko" ? WTHR_KR_DL : WTHR_ENG_DL}</h6>
                    </section>
                  </div>
                ))}
          ...
          <article className="text-center flex flex-col items-center">
            <Image src={`/images/weather/${weather}.svg`} onError={handleWeatherImageError} width={0}height={0} alt="" className="w-20 mb-1" />
            <h6 className="text-sm text-gray-700">{router.locale=="ko" ? koreanWeatherExplain : weatherExplain}</h6>
          </article>
        </section>
      </header>
