...
const MultiSelect = ({
  ...
  return (
    <article className="flex gap-4 items-center relative justify-between min-w-80">
      <h6 className="text-sm text-[#111111]">{title}</h6>
      <div
      ...
      {isActive && (
        <div
          className={`z-10 absolute top-11 right-0 bg-white py-3 px-2 border-gray-200 border rounded-20 shadow-[0_0_12px_0_rgba(121,120,132,0.15)] ${
            isLarge ? "w-[371px]" : isMidium ? "w-[271px]" : "w-[253px]"
          }`}
        >
          {data.map(({ id, title }) => (
