...
import { UserInfo, SearchList, SelectAsset,UserPortfolio } from "interfaces/portfolio";
...
const AddAssets = ({page, limit, setPage, setLimit,rowCount,currencySelect,session, setIsOpenAddModal,userProfile,isValidating,mutate,search,setSearch,onSearch,searchList,searchValid  }: Props) => {
  ...
  const handleSelect = ({name,ticker,quantity,krName}:SelectAsset) => {
    ...
    if(userProfile?.portfolio && userProfile?.portfolio.some((asset:UserPortfolio)=>asset['ticker']==ticker)){
      toast(router.locale=="ko" ? "포트폴리오에 이미 포함된 자산입니다" :"You already have the asset on your portfolio",{ hideProgressBar: true, autoClose: 2000, type: "error" })
      return}




    if(userProfile?.portfolio && (userProfile?.portfolio.length + selectIndex.length)>=10){
      toast(router.locale=="ko" ? "10개 초과의 자산을 추가하실 수 없습니다" :"You can't add more than 10 assets",{ hideProgressBar: true, autoClose: 2000, type: "error" });
    return}


    if (selectIndex.length>=10){toast(router.locale=="ko" ? "10개 초과의 자산을 추가하실 수 없습니다" :"You can't add more than 10 assets",{ hideProgressBar: true, autoClose: 2000, type: "error" }); return}
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>, name: string, ticker: string) => {
...
const submitHandler=async()=>{
    if (selectIndex.some(asset=>asset['quantity']<=0)) {toast(router.locale=="ko" ? "최소 1개 이상의 자산을 추가하여야 합니다" :"your asset should be more than 0",{ hideProgressBar: true, autoClose: 2000, type: "error" }); return}
    let enteredInput={}
    if(userProfile?.portfolio && userProfile?.portfolio.length>9){toast(router.locale=="ko" ? "10개 초과의 자산을 추가하실 수 없습니다" :"Your assets are limited to 10",{ hideProgressBar: true, autoClose: 2000, type: "error" });  return }
    if(userProfile?.portfolio && (userProfile?.portfolio.length + selectIndex.length)>10){toast(router.locale=="ko" ? "10개 초과의 자산을 추가하실 수 없습니다" :"You can't add more than 10 assets",{ hideProgressBar: true, autoClose: 2000, type: "error" }); return}
  ...
  try {
    ...
    const data=await fetch(`/api/auth/addUserInfo?session=${session.user.email}`,{
        method:"POST",
        headers:{"content-type":"application/json"},
        body:JSON.stringify({enteredInput})
    }).then((res)=>{if(res.ok){setSearch("");setIsOpenAddModal(false);  toast(router.locale=="ko" ? "포트폴리오에 성공적으로 자산이 추가됐습니다" :"Successfully added assets to your portfolio",{ hideProgressBar: true, autoClose: 2000, type: "success" });mutate()}else{toast(router.locale=="ko" ? "네트워크 에러가 발생했습니다" :"Fetch Error",{ hideProgressBar: true, autoClose: 2000, type: "error" })}})
    
    toast(router.locale=="ko" ? "리스크 계산이 완료됐습니다" :"Calculation completed!", {
    ...
    toast(router.locale=="ko" ? "리스크 계산에 실패했습니다" :"Fetch Error", {
      hideProgressBar: true,
      autoClose: 2000,
      type: "error",
    });
  ...
  return (
    ...
    <main className="z-20 shadow-lg absolute text-center text-[#111111] bg-white px-8 pt-8 pb-3 rounded-20 top-1/2 left-1/2 translate-x-half translate-y-half border min-w-[820px]  max-w-[1018px] w-full">
      <header className="flex mb-5 ">
        <section className="flex-1 flex items-center">
          ...
          {userProfile?.portfolio ? (userProfile?.portfolio.length + selectIndex.length)>=2 && selectIndex.length>0 ?
          <button onClick={()=>{setAddAsset(true)}} className="bg-[#0198FF] h-10 py-2.5 px-5 rounded-[60px] text-white text-sm hover:bg-[#0085E6] disabled:bg-[#D1D5DB]">
            <h1 className={'lg:text-sm text-xs'}>{t('assetAdd')}</h1>
          </button>: 
          <button disabled onClick={()=>{setAddAsset(true)}} className="bg-[#0198FF] h-10 py-2.5 px-5 rounded-[60px] text-white text-sm hover:bg-[#0085E6] disabled:bg-[#D1D5DB]">
          <h1 className={'lg:text-sm text-xs'}>{t('assetAdd')}</h1>
        </button> :
      ...
      <h1 className={'lg:text-sm text-xs'}>{t('assetAdd')}</h1>
        ...
        {userProfile?.portfolio ? <p className={'font-medium text-gray-400 ml-10'}>{userProfile?.portfolio?.length + selectIndex.length}/10</p> : <p className={'font-medium text-gray-400 ml-5'}>{selectIndex.length}/10</p> }
        {!userProfile?.portfolio && selectIndex.length<2  ? <p className={' text-red-500 ml-5 lg:ml-10 lg:text-sm text-xs'}>{router.locale=="ko" ? "자산을 2개 이상 선택해주시길 바랍니다" : "Select at least 2 assets to build your portfolio"}</p> : <p className={'font-medium text-gray-400 ml-5'}></p> }
        {userProfile?.portfolio && (userProfile?.portfolio?.length + selectIndex?.length)<2  ? <p className={'text-red-500 ml-5 lg:ml-10 lg:text-sm text-xs'}>{router.locale=="ko" ? "자산을 2개 이상 선택해주시길 바랍니다" : "Select at least 2 assets to build your portfolio"}</p> : <p className={'font-medium text-gray-400 ml-5'}></p> }
    ...
    {!searchValid ?    <section className="overflow-auto h-full customScrollBar ">
        {searchList.map(({EXP_CVaRNTS,ADJ_CLOSE_KRW,HR_ITEM_NM,LOC,LV_DSCP_KR,LV_DSCP_ENG,ITEM_ENG_NM,ITEM_KR_NM,ITEM_CD_DL,CVaR_LV,CVaRNTS,ADJ_CLOSE,WTHR_ENG_NM,WTHR_ENG_DL,ADJ_CLOSE_USD,WTHR_KR_DL, CVaR_LV_KR}:SearchList , i:number) => (
          <article
            key={i}
          ...
          >
            {selectIndex.some(asset=>asset['ticker']==ITEM_CD_DL) ? (
              <Image
                ...
                onClick={() => handleSelect({name:ITEM_ENG_NM,ticker:ITEM_CD_DL,quantity:0, krName:ITEM_KR_NM})}
              />
            ) : (
              <Image
                src={checkbox}
            ...
            <Image loading="eager" unoptimized  width={10} height={10} quality={100} onError={(event)=>handleImageError(event,HR_ITEM_NM)} src={`/images/logos/${ITEM_CD_DL}.png` || ""} alt="" className="h-10 w-10 mr-3 "/>                  <div>
                <h6 className="text-md w-[150px] truncate" title={router.locale=="ko" ? ITEM_KR_NM : ITEM_ENG_NM}>{router.locale=="ko" ? ITEM_KR_NM : ITEM_ENG_NM}</h6>
                <p className="text-gray-500 text-xs">{ITEM_CD_DL}</p>
              </div>
            </div>
            <div
            data-tooltip-id="riskLevel"
            data-tooltip-content={router.locale=="ko" ? LV_DSCP_KR : LV_DSCP_ENG}
            ...
            <div className="text-center text-[#DF1525] w-[124px] mr-11 font-bold">
              -{EXP_CVaRNTS.toLocaleString('en-us',{minimumFractionDigits: 2, maximumFractionDigits: 2})}%
            </div>
            <div className="text-center w-[140px] mr-11 text-[#111111] font-medium lg:text-md text-sm">
            <p>{currencySelect===1  ? "￦" : "$"}{" "}{currencySelect===1 ? ADJ_CLOSE_KRW.toLocaleString('en-us',{minimumFractionDigits: 0, maximumFractionDigits: 0}) :ADJ_CLOSE_USD?.toLocaleString('en-us',{minimumFractionDigits: 2, maximumFractionDigits: 2})}</p>
            ...
            <div className="rounded-20 bg-gray-100 py-[2px] pl-3 pr-6 w-[174px] mx-auto flex items-center justify-between ">
              <Image src={`/images/weather/${WTHR_ENG_NM}.svg`} width={24} height={24} alt="" />
  ...
  <svg aria-hidden="true" className="my-20 w-12 h-12  relative  text-gray-200 animate-spin dark:text-gray-600 fill-blue-400" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <p className="text-sm text-[#6B7280] text-end "> {searchList.length>0 && !isValidating ? udtDate : ''}</p>
    ...
    <section className="overflow-auto h-[260px] customScrollBar">
      {selectIndex.map(({ name, ticker,quantity,krName }, i) => (
        <article className="flex items-center py-3" key={i}>
          ...
          <Image loading="eager" unoptimized  width={50} height={50} quality={100} onError={handleWeatherImageError} src={`/images/logos/${ticker}.png` || ""} alt="" className="h-10 w-10 mr-3 "/>                <div>
              <h6 className="mb-1 w-[130px] truncate" title={router.locale=="ko" ? krName: name}>{router.locale=="ko" ? krName: name}</h6>
          ...
          <input
            ...
            type="number"
            min="0"
            onChange={(e)=>{handleChange(e,name,ticker)}}
