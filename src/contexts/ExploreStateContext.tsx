...
const ExploreStateContext = createContext<ExploreStateType | undefined>(undefined);

// Explore 상태 훅
export const useExploreState = (): ExploreStateType => {
  const context = useContext(ExploreStateContext);
  if (!context) {
    throw new Error("useExploreState must be used within a ExploreStateProvider");
  }
  return context;
};
...
export const ExploreStateProvider = ({ children }:ExploreStateProviderProps) => {
