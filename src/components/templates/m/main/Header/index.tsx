...
import Carousels from "components/organisms/m/Carousels";
...
const Header = ({carouselData,carouselType,setCarouselType,carouselValid}:Props) => {
  ...
  return (
    <main className="pt-8 w-full min-w-[360px]">
      ...
      type="button" className="flex items-center mb-7 py-2.5 px-14 mr-2  text-md font-bold text-[#0198FF] focus:outline-none bg-white rounded-full border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-200 ">
      {t('headerButton')}
      <Image
                src={blackArrow}
                alt=""
                className="w-4 ml-2 -rotate-90"
              />
      </button>
      ...
      <ul className="flex justify-center gap-2.5 mb-7 ">
        {MENUS.map(({ id, title,koreanTitle }:{id:number;title:string;koreanTitle:string;}) => (
          ...
          >
            <h1 className="">{router.locale =="ko" ? koreanTitle: title}</h1>
          </li>
        ))}
      </ul>
      ...
      <CarouselsWithAd carouselData={carouselData}/>
