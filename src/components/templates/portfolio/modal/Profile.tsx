...
const Profile = ({session, setIsOpenProfileModal, setIsOpenDeleteModal,userProfile,isValidating,mutate  }: Props) => {
  ...
  const uploadFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    let { data } = await axios.post(`/api/auth/uploadProfile?session=${session.user.email}`, {
    ...
    setFile(null);
    toast(router.locale=="ko" ? "프로필 이미지 변경이 완료됐습니다" :"Successfully Changed your profile image",{ hideProgressBar: true, autoClose: 2000, type: "success" });
    mutate()
    // sessionUpdate()
  }
  };
  ...
  const saveClick=async()=>{
    ...
    if((userName=="" || userName==userProfile?.name) && defaultImage==0){
      setIsOpenProfileModal(false)

    }

  else if (userName && userName.length>49) {
      return toast(router.locale=="ko" ? "이름은 50자 이내여야 합니다" :"Your name should be lesser than 50 words",{ hideProgressBar: true, autoClose: 2000, type: "error" })
     }
  ...
  }).then((res)=>{if(res.ok){toast(router.locale=="ko" ? "프로필 변경이 완료됐습니다" :"Successfully Changed your Profile Name",{ hideProgressBar: true, autoClose: 2000, type: "success" });mutate()}else{toast(router.locale=="ko" ? "네트워크 에러가 발생했습니다" :"Fetch Error",{ hideProgressBar: true, autoClose: 2000, type: "error" })}})
    setIsOpenProfileModal(false)
    mutate()
    // sessionUpdate()
  }
    ...
    <main className="z-20 absolute bg-white p-8 rounded-20 top-1/2 left-1/2 translate-x-half translate-y-half border max-w-[415px]">
     ...
     <div className={'flex justify-center items-center'}>
        <article className="relative w-[100px]  mb-6  ">
          <Image  src={userProfile?.profileImage || '/images/logos/errorLogo.png'}
        ...
        <article className={'flex border border-gray-100 gap-2 mb-5 items-center justify-center p-3 '}>
          {[1,2,3,4].map((i)=>
          <div key={i} className={'relative'}>
        <Image  key={i} src={`/images/users/default${i}.svg`}
          onError={handleImageError}
        ...
        <article className="mb-4 flex items-center">
          <p className="text-sm w-[78px] text-gray-500 font-medium">{t('profileName')}</p>
          <input
          type="text"
