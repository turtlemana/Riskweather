...
const Chart = ({currencySelect, setCurrencySelect,session, setIsOpenAddModal,mutate,ptfData,userProfile }: Props) => {
    ...
    <main className="bg-white px-5 pt-7 rounded-20 text-sm">
      ...
      <section>
        <article className="flex gap-3 mb-7 items-center">
          <h1 className="text-2xl flex-1 text-[#111111]">{t('portfolioTitle')}</h1>
          <div
        ...
        >
          ...
          <Image src={refresh} onClick={()=>{setIsRefreshModalOpen(true)}}  alt="" className="cursor-pointer w-6" />
          {/* <Image src={pen} alt="" className="cursor-pointer w-6" /> */}
          <Image src={trash} onClick={()=>{setIsDeleteModalOpen(true)}}  alt="" className="cursor-pointer w-6" />
        </article>
        <article className="flex mb-3 items-center text-gray-500 text-2xl">
          ...
          <h6 className="text-[#111111]">{currencySelect===1 ? "￦" +" " + krTotalAmount?.toLocaleString('en-us',{minimumFractionDigits: 0, maximumFractionDigits: 0}) : "$" +" " +totalAmount?.toLocaleString('en-us',{minimumFractionDigits: 2, maximumFractionDigits: 2})} </h6>
        </article>
        <article className="flex mb-3 items-center text-gray-500 text-2xl">
          <p className="text-xs font-medium flex-1">{t('portfolioLoss')}</p>
          <h1 className={`${userProfile?.portfolioResult  ? "text-[#DF1525]" : "text-black"}`}>
        ...
        >
          <h6 className="text-xs">{router.locale==="ko" ? 
        ...
        </div> : <h1 className={'text-black'}>{"X"}</h1>}
  ...
  <div key={i} className="flex text-[#111111] mb-4 items-center">
              <div className={`w-5 h-5 ${COLORS[i]} rounded-[4px] mb-px mr-3`} />
              <h6 className="flex-1 text-sm">{router.locale=="ko" ? krName : name}</h6>
              <h6>{((total/totalAmount) *100).toLocaleString('en-us',{minimumFractionDigits: 2, maximumFractionDigits: 2})}{"%"}</h6>
            </div>
          ))
        ) : (
          <>
            ...
            <div className="flex text-[#111111] mb-4 items-center">
              ...
              <h6>0%</h6>
                    ...
                    <p className={'text-end text-gray-400 text-sm mt-5'}>{userProfile?.portfolioTime ? router.locale==="ko" ? "리스크 측정 시간 : " +  userProfile?.portfolioTime : "Risk Calculated : " +  userProfile?.portfolioTime :   ""} </p>

      </section>
      {isRefreshModalOpen && (
      
      <div className={'z-30 sticky bottom-[100px] text-center text-[#111111] bg-white rounded-20 '}>
