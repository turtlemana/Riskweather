...
const Top = ({EXP_CVaRNTS, EXP_CVaRNTS_95, EXP_VaRNTS, EXP_VaRNTS_95,exchg,krName,cat,forcast, name,ticker,varGauss, varGauss95, tailRisk95, cVaRNTS95, koreanRiskLevel, koreanWeatherExplain, tailRisk,usdPrice,riskLevel,maxLoss,price,currency,priceChange,date,weather,weatherExplain }: Props) => {
  ...
  const forcastData=useMemo(()=>{
    ...
    if (!tmpForcast || tmpForcast.length===0 || tmpForcast.message==="Can't find data")return
    if(router.locale==="ko"){tmpForcast[0].DATE_PlUS="오늘"}
    else {tmpForcast[0].DATE_PlUS="Today"}
  ...
  return (
    <main className="py-4 bg-white pb-[44px]">
      ...
      <Image onError={(event)=>handleImageError(event,exchg)} unoptimized src={`/images/logos/${ticker}.png` || ""} alt="" quality={100} width={50} height={50}  className="w-10 h-10 " />
        <div className="flex-1 ml-3">
          <h6 className="">{router.locale=="ko" ? krName : name}</h6>
          <p className="text-[#616161] text-xs">{ticker}</p>
        </div>
        <div className={`py-1 h-6 px-4 rounded-20 text-center ${COLORS[riskLevel]}`}>
          <h6 className="h-5 text-xs mt-px">{router.locale=="ko"? koreanRiskLevel :  riskLevel}</h6>
        </div>
      </section>
      <section className="flex pr-5">
        <article className="px-5 flex-1">
          ...
          <h6 className="text-xl text-[#111111] mb-1">
            {price?.toLocaleString('en-us',{minimumFractionDigits: 0, maximumFractionDigits: 2})}{" "}{cat=="Index" ? "" : currency}
          </h6>
          <h6 className={` mb-5  ${priceChange > 0 ? "text-[#3CB043]" : "text-[#DF1525]"} `}>{priceChange.toFixed(2)}%</h6>
        ...
        <article>
          ...
          <div className="text-center flex flex-col items-center w-[135px] h-[96px] bg-gray-50 rounded-[8px] mb-3">
            <Image onError={handleWeatherImageError} src={`/images/weather/${weather}.svg`} width={0} height={0} alt="" className="w-[64px] h-auto mb-1" />
            <h6 className="text-xs text-gray-700">{router.locale=="ko" ? koreanWeatherExplain : weatherExplain}</h6>
          </div>
          <div
            className={
          ...
          >
            ...
            {isActiveWeeklyModal && (
              <div
              //@ts-ignore 
              ref={modalRef} className="max-w-[269px] z-30 absolute p-6 bg-white shadow-[0_0_12px_0_rgba(121,120,132,0.15)] top-8 right-0 rounded-20 border border-gray-100">
                <div className="flex items-center mb-7 w-[221px]">
                  <h6 className="flex-1">{t('headerForecast')}</h6>
                  <Image src={close} alt="" className="w-6" />
                </div>
                {forcastData?.map(({DATE_PlUS, WTHR_ENG_NM,WTHR_ENG_DL,WTHR_KR_DL }:Forcast,i:number) => (
                  <div className="flex items-center mb-3" key={i}>
                    <p className="flex-1 mt-1 text-sm font-bold">{DATE_PlUS}</p>
                    <section className="text-center flex items-center justify-between bg-gray-100 rounded-20 pl-3 pr-4 py-0.5">

                    <Image onError={handleWeatherImageError} src={`/images/weather/${WTHR_ENG_NM}.svg`} width={50} height={50} alt="" quality={100} className="w-8 h-8" />
                    <p className="text-xs font-medium text-gray-600">{router.locale=="ko" ? WTHR_KR_DL : WTHR_ENG_DL}</p>
        ...
        <article className="flex items-center gap-4">
            ...
            >
              <div className="">
                ...
                <h6 className="text-gray-700 text-sm">{(tailRisk/100).toFixed(2) }</h6>
              </div>
            ...
            >
              <div className="">
                ...
                <h6 className="text-gray-700 text-sm">{(EXP_CVaRNTS).toFixed(2)+"%"}</h6>
              </div>
            ...
            >
              <div className="">
                ...
                <h6 className="text-gray-700 text-sm">{(EXP_VaRNTS).toFixed(2)+"%"}</h6>
              </div>
             
             
            </div>
            <div data-tooltip-id="riskIndexExplain" className={'items-center mr-4'} >
            <BsQuestionCircle   />
          </div>
        </article> : 
                <article className="flex items-center gap-4">
                ...
                >
                  <div className="">
                    <p className="text-gray-400 text-xs mb-1">RW INDEX</p>
                    <h6 className="text-gray-700 text-sm">{(tailRisk95/100).toFixed(2) }</h6>
                  </div>
                ...
                >
                  <div className="">
                    <p className="text-gray-400 text-xs mb-1">CVAR</p>
                    <h6 className="text-gray-700 text-sm">{(EXP_CVaRNTS_95).toFixed(2)+"%"}</h6>
                  </div>
                ...
                >
                  <div className="">
                    <p className="text-gray-400 text-xs mb-1">VAR</p>
                    <h6 className="text-gray-700 text-sm">{(EXP_VaRNTS_95).toFixed(2)+"%"}</h6>
                  </div>
