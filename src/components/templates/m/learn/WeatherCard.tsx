...
const WeatherCard = () => {
  ...
  return (
    <main  className="bg-white py-6 px-5 max-w-1320">
      <h6   className="text-[#111111] mb-9">{router.locale=="ko" ? "날씨 카드" : "Weather Card"}</h6>
      ...
      <section className="flex items-center mb-8 justify-end gap-5 text-xs font-medium text-[#111111]">
        <article>
          <p>자산의 내재 리스크 등급</p>
          <p className="text-gray-400 mt-1">{router.locale==="ko" ? '(안전, 적절, 높음, 매우높음)' : '(Low, Moderate, High, Very High)'}</p>
        </article>
      ...
      <Image src={router.locale==="ko" ? weatherCardKr : weatherCard} alt="" className="mx-auto mb-4" id="Importance of RISK" />
    </main>
  );
};
