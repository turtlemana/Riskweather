...
const Map = ({locData,locValid,mapSelect,setMapSelect,mapIndexData}) => {
  ...
  const handleDragOver = (e) => {
    ...
    touchStartPoint.current = screenY;
    
   // 리스트 컴포넌트의 높이를 전체 화면으로 변경
  };

  const handleDragEnd = (e) => {
    const { screenY } = e.changedTouches[0];
    const toBottom = touchStartPoint.current < screenY;
    if (toBottom) {
      setIsFull((prev) => !prev);
    } else if (!toBottom) {
      setIsFull((prev) => !prev);
    }
     // 리스트 컴포넌트의 높이를 원래대로 변경
  };
  ...
  useEffect(()=>{
    ...
    for (let i=0; i<markers.length; i++){
      for (let j=0; j<mapIndexData.length; j++){
        if(markers[i].ticker == mapIndexData[j].ticker){
         Object.assign(markers[i],mapIndexData[j])
    ...
    for (let i=0; i<sideBarMarkers.length; i++){

        for (let j=0; j<mapIndexData.length; j++){
        if(sideBarMarkers[i].ticker == mapIndexData[j].ticker){
         Object.assign(sideBarMarkers[i],mapIndexData[j])

          // sideBarMarkers[i].weather=mapIndexData[j].WTHR_ENG_NM
          // sideBarMarkers[i].weatherExplain=mapIndexData[j].WTHR_ENG_DL
          // sideBarMarkers[i].assetName=mapIndexData[j].ITEM_ENG_NM
          // sideBarMarkers[i].risk=(mapIndexData[j].CVaR_LV)
          // sideBarMarkers[i].maxLoss=(mapIndexData[j].CVaRNTS)
          break
        //   markers[i].onClick=(mapData[j].onClick)
        }
      }
...
(<Marker key={i} coordinates={coordinates}  >
 {/* <svg viewBox="142 0 850 850" >
    <foreignObject x="0" y={markerOffset}  width="180" height="80">
      <div onClick={()=>{setMapSelect(loc);setSideMarkers((marker)=>(marker.filter(country=>country.name==name)));setPosition((pos)=>({...pos,zoom:4,coordinates: country=="France" ? sideBarMarkers[4].coordinates : coordinates, name:country}))}} className={`cursor-pointer w-full relative z-20 font-semibold  items-center bg-[#FFFFFF] border p-3 rounded-2xl flex text-[5px] opacity-100 justify-center`}>
      <Image className={'border'} width={20} height={25} src={`/images/flags/${countryCode}.svg`} alt={`${countryCode}`}></Image>
       <p className={`ml-5 text-xs truncate`}>{loc}</p>
...
:""
             ...
             <div className={'flex'}
                onClick={()=>{setMapSelect("");setPosition((pos)=>({...pos, zoom:1.05, coordinates:[25,20], name:""}));setSideMarkers(sideBarMarkers)}}
              > 
              <Image
                src={arrow}
                alt=""
    ...
    <div 
      ...
      {/* {modalPage === 2 && (
        locData.length>2 &&
        locData.map(({ITEM_CD_DL,ITEM_ENG_NM,WTHR_ENG_DL,WTHR_ENG_NM,CVaRNTS,CVaR_LV})=> 
       
        <section  key={ITEM_CD_DL}         onClick={()=>{setSideMarkers((prev)=>[{...prev[0],assetName:ITEM_ENG_NM, ticker:ITEM_CD_DL, weather:WTHR_ENG_NM, weatherExplain:WTHR_ENG_DL, risk:CVaR_LV, maxLoss:CVaRNTS}])}}
        className="absolute bottom-0 overflow-scroll w-full shadow-[4px_0_16px_0_rgba(121,120,132,0.15)] rounded-[20px_20px_0_0] pb-[60px]">
          
          
          <div className="flex items-center py-4 px-5 text-sm border-b border-gray-100">
            <Image
              src={arrow}
              alt=""
              className="rotate-180 w-4 cursor-pointer mr-1 mb-[2px]"
              onClick={() => setModalPage(1)}
            />
            <p className="flex-1 text-gray-900">Go back</p>
            <Image
              src={arrow}
              alt=""
              className="rotate-90 w-5 cursor-pointer"
              onClick={() => setModalPage(0)}
            />
          </div>
          <article className="flex-1 py-4 px-5">
            <section className="flex items-start mb-3">
              <img src={`/images/logos/${ITEM_CD_DL}.png`} alt="" className="w-10 h-10 mr-3 items-center rounded-full" />
              <div className="flex-1">
                <h6>{ITEM_ENG_NM}</h6>
                <p className="text-gray-500 text-xs">{ITEM_CD_DL}</p>
              </div>
              <div className={`py-1 px-3 rounded-20 ${COLORS[CVaR_LV]}`}>
                <h6 className="text-xs">{CVaR_LV}</h6>
              </div>
            </section>
            <section className="bg-gray-100 rounded-20 py-2 px-3 flex items-center h-8 text-medium text-xs ">
              <p className="text-gray-400 mr-1.5">Max loss</p>
              <p className="text-[#DF1525] flex-1">-{CVaRNTS.toFixed(4)}%</p>
              <Image
                src={`/images/weather/${WTHR_ENG_NM}.svg`}
                alt=""
                ...
                height={0}
              />
              <p className="text-gray-600 font-medium">{WTHR_ENG_DL}</p>
            </section>
          </article>
        </section>)
      )} */}
