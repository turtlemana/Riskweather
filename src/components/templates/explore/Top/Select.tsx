...
interface Props {
  ...
  data: {id:number;title:string;koreanTitle:string;}[] | 
...
const Select = ({
  ...
  return (
    <article className=" flex gap-4 items-center relative justify-between min-w-80">
      <label htmlFor={title} className="lg:text-sm text-xs font-bold text-[#111111]">{router.locale=="ko" ? koreanTitle :title}</label>
      <select id={title}
      value={defaultData}
        onClick={() => setIsActive(true)}
        onBlur={()=>setIsActive(false)}
        onChange={selectHandler}
        disabled={title=="Location" ? type=="Crypto" ? true : false : type=="Crypto" || type=="Index" ? true : false}
        ...
        className={`disabled:bg-gray-200  cursor-pointer text-xs border rounded-20 max-h-10 py-2.5 px-3.5 flex justify-between hover:border-[#0198FF] ${
          isActive
            ...
            : "border-gray-200 "
        } ${isLarge ? "w-[371px]" : isMidium ? "w-[271px]" : "w-[180px] lg:w-[253px]"}`}
      >
          ...
          {data.map(({ id, title,koreanTitle }) => (
            ...
            >
                ...
                {router.locale=="ko" ? koreanTitle : title =="Partly_cloudy"  ? "Partly Cloudy" : title == "Mostly_cloudy" ? "Mostly Cloudy" : title}
