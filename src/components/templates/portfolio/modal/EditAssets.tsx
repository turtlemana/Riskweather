...
const EditAssets = ({session, setIsOpenAddModal,userProfile,isValidating,mutate  }: Props) => {
  ...
  const submitHandler=async()=>{
    ...
    let enteredInput={}
    if(userProfile?.portfolio && userProfile?.portfolio.length>10){toast(router.locale=="ko" ?  "10개 이상의 자산을 추가하실 수 없습니다" :"Your assets are limited to 10",{ hideProgressBar: true, autoClose: 2000, type: "error" });  return }
    setIsLoading(true);
      ...
      <section className="overflow-auto h-[260px] customScrollBar">
        ...
        userProfile?.portfolio.map(({ name, ticker, quantity,krName }:UserPortfolio, i:number) => (
          <article className="flex items-center py-3" key={i}>
            ...
            <Image loading="eager" unoptimized  width={10} height={10} quality={100} onError={handleImageError} src={`/images/logos/${ticker}.png` || ""} alt="" className="h-10 w-10 mr-3 "/>                  <div>
