...
const Chart = ({currencySelect, setCurrencySelect,session, setIsOpenAddModal,mutate,ptfData,userProfile }: Props) => {
  // const COLORS = ["bg-[#89aaff]", "bg-[#6691ff]", "bg-[#4277ff]", "bg-[#1e5dff]","bg-[#0046f9]","bg-[#003cd6]","bg-[#0032b2]","bg-[#00288e]","bg-[#001e6b]","bg-[#001447]"];
  const COLORS=["bg-chart1","bg-chart2","bg-chart3","bg-chart4","bg-chart5","bg-chart6","bg-chart7","bg-chart8","bg-chart9","bg-chart10"]
  ...
  const chartData=useMemo(()=>{
    ...
    let sortedData : PortfolioData[] =ptfData.sort((a:PortfolioData,b:PortfolioData) => b.total - a.total)
    if (router.locale =="ko"){
    sortedData =  sortedData.map((asset)=> {return {...asset,"name":asset.krName, "enName":asset.name}})
    }
    return sortedData
...
const refreshHandler=async()=>{
  if(ptfData.length==0){
    toast(router.locale=="ko" ? "자산을 먼저 추가해주세요":"Please add your assets",{ hideProgressBar: true, autoClose: 2000, type: "error" })
    return
  }
  if(ptfData.length==1){
    toast(router.locale=="ko" ? "최소 2개 이상의 자산이 필요합니다":"Please add at least 2 assets to calculate your risk",{ hideProgressBar: true, autoClose: 2000, type: "error" })
    return
  }
  setIsLoading(true)
  ...
  try {
    ...
    }).then((res)=>{if(res.ok){setIsOpenAddModal(false);mutate(); }else{toast(router.locale=="ko" ? "네트워크 에러가 발생했습니다" :"Fetch Error",{ hideProgressBar: true, autoClose: 2000, type: "error" })}})
...
const deleteHandler=async()=>{
  ...
  enteredInput={
    ...
    const data=await fetch(`/api/auth/addUserInfo?session=${session.user.email}`,{
      ...
      headers:{"content-type":"application/json"},
      body:JSON.stringify({enteredInput})
  }).then((res)=>{if(res.ok){setIsLoading(false);setIsOpenAddModal(false);
    mutate();  
    toast(router.locale=="ko" ? "포트폴리오를 삭제했습니다" :"Successfully deleted your portfolio",{ hideProgressBar: true, autoClose: 2000, type: "success" })}else{setIsLoading(false);toast(router.locale=="ko" ? "네트워크 에러가 발생했습니다" :"Fetch Error",{ hideProgressBar: true, autoClose: 2000, type: "error" })}})
    ...
    <main className="bg-white px-6 py-4 rounded-20 max-w-[650px] flex-1 max-h-[822px]">
      ...
      <section className={''}>
        <article className="flex gap-6 mb-7 items-center">
          <h1 className="text-3xl mt-1 flex-1 text-[#111111]">{t('portfolioTitle')}</h1>
          <div
          className="relative flex gap-4 bg-gray-100 rounded-[14px] px-2 py-1.5 w-[128px] h-7 justify-between items-center cursor-pointer "
        ...
        >
          ...
          <Image src={refresh} onClick={()=>{setIsRefreshModalOpen(true)}} alt="" className="cursor-pointer" />
          {/* <Image src={pen} alt="" className="cursor-pointer" /> */}
          <Image src={trash} onClick={()=>{setIsDeleteModalOpen(true)}} alt="" className="cursor-pointer" />
        </article>
        <article className="flex gap-6 mb-5 items-center text-gray-500 text-4xl">
          ...
          <h1 className="text-[#111111]">{currencySelect===1 ? "￦" +" " + krTotalAmount?.toLocaleString('en-us',{minimumFractionDigits: 0, maximumFractionDigits: 0}) : "$" +" " +totalAmount?.toLocaleString('en-us',{minimumFractionDigits: 2, maximumFractionDigits: 2})}</h1>
        </article>
        <article className="flex gap-6 mb-5 items-center text-gray-500">
          ...
          >
            {userProfile?.portfolioResult ? "- "+userProfile.portfolioResult.toFixed(2) +"%" : "X"} 
          </h1>
        </article>
        <article className="flex gap-6 mb-5 items-center text-gray-500">
          <p className="text-xl font-medium flex-1">{t('portfolioLevel')}</p>
         { userProfile?.portfolioLevel ? <div 
          className={`py-1 px-3 rounded-20 ${PORTCOLORS[userProfile?.portfolioLevel]} max-w-fit mx-auto`}
        >
          <h6 className="text-xl">{router.locale==="ko" ? 
          userProfile?.portfolioLevel==="Low volatility" ? "변동성 낮음" :
          userProfile?.portfolioLevel==="Moderate volatility" ? "변동성 적정" :
          userProfile?.portfolioLevel==="High volatility" ? "변동성 높음" :
          userProfile?.portfolioLevel==="Very High volatility" ? "변동성 매우 높음" :
          "X"
          : userProfile?.portfolioLevel}</h6>
        </div> : <h1 className={'text-3xl text-black'}>{"X"}</h1>}
    
        </article>
      </section>
      <section className="mt-10  w-[390px] h-[290px] flex justify-center items-center mx-auto my-10">
        {!isLoading ?
        ptfData?.length>0 ? <AssetChart data={chartData}/> : <Image src={emptyChart} alt="Empty Chart"/> 
      ...
      <section className="mt-5 border  border-gray-200 pt-4 pb-2 px-5 w-[234px] rounded-20 mb-4 ml-2 h-[136px] overflow-y-scroll customScrollBar">
        {chartData.length !== 0 ? (
          ptfData.map(({ name, ticker, quantity,total,krName }:PortfolioData,i:number) => (
            <div
              key={i}
              className={`flex text-[#111111] text-sm mb-2 items-center `}
            >
              <div className={`${COLORS[i% COLORS.length]} w-3 h-3   rounded-[50%] mb-[2px] mr-2 `} />
            <h6 className="flex-1 w-[100px] truncate mr-2" title={router.locale=="ko" ? krName : name}>{router.locale=="ko" ? krName : name} </h6>
              <h6 className="font-medium">{((total/totalAmount) *100).toLocaleString('en-us',{minimumFractionDigits: 2, maximumFractionDigits: 2})}{"%"}</h6>
            </div>
          ))
      ...
      <p className={'mb-5 text-end  text-gray-400  text-sm'}>{userProfile?.portfolioTime ? router.locale==="ko" ? "리스크 측정 시간 : " +  userProfile?.portfolioTime : "Risk Calculated : " +  userProfile?.portfolioTime :   ""} </p> 
    
      </div>
      {isRefreshModalOpen && (
      
      <div className={'absolute text-center text-[#111111] bg-white rounded-20 top-1/2 left-1/2 translate-x-half translate-y-half '}>
