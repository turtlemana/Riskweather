...
const Items = ({EXP_CVaRNTS,curr,exchg,loc,krName, name,ticker,riskLevel,riskDescriptionEn, riskDescriptionKr, koreanRiskLevel, koreanWeatherExplain, usdPrice,cat,priceChg,maxLoss,price,currency, priceChange,chartData,weather,weatherExplain }: Props) => {
    const router=useRouter()

    const handleImageError = useHandleImageError();

  return (
    <tr  className="border-b border-gray-100 cursor-pointer" onClick={()=>{router.push({pathname:`/detail/${ticker}`})}}>
      ...
      <h6 className="mr-3 text-md w-[150px] truncate" title={router.locale=="ko" ? krName : name}>{router.locale=="ko" ? krName : name}</h6>
        <p className="text-gray-500 text-xs">{ticker}</p>
      </td>
      <td>
        <div data-tooltip-id="riskLevel" data-tooltip-content={router.locale=="ko" ? riskDescriptionKr : riskDescriptionEn}
      ...
      <td className="text-center text-[#DF1525] font-semibold">-{EXP_CVaRNTS?.toFixed(2)}%</td>
      <td className="text-center font-medium">{cat=="Index" ? "" : curr==="KRW" ? "￦" : "$"}{" "}{curr==="KRW" ? price.toLocaleString('en-us',{minimumFractionDigits: 0, maximumFractionDigits: 0}) :  price.toLocaleString('en-US',{minimumFractionDigits: 2, maximumFractionDigits: 2})}</td>
      ...
      <td className={`font-semibold text-center ${priceChg>0 ? "text-[#3CB043]" : "text-[#DF1525]"}`}>{priceChg.toFixed(2)}%</td>
      ...
      <td>
        <div className="rounded-20 bg-gray-100 py-[2px] pl-3 pr-4 max-w-[174px] mx-auto flex items-center justify-between ">
          ...
          <p className="text-xs text-gray-600 font-medium">{router.locale=="ko" ? koreanWeatherExplain : weatherExplain}</p>
        </div>
      </td>
      <td>
        <Image src={arrow} alt="" />
