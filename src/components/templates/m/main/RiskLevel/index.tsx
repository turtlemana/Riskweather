...
const RiskLevel = ({riskLevelData,riskLevelType,setRiskLevelType,riskLevelValid}:Props) => {
  ...
  return (
    <main className="py-7 border-y-[6px] border-gray-50 w-full">
      <div className="px-5 mx-auto bg-white rounded-20 overflow-auto">
        <div className={'flex justify-between'}>
        <h1
        onClick={()=>{dispatch({ type: 'RESET_VALUE' }); router.push({pathname:`/${router.locale}/explore`, query:{type:riskLevelType}})}}
        className="flex items-center text-[28px] text-[#111111] mb-5">{t("riskLevelTitle")}
                  <Image
                src={blackArrow}
                alt=""
                className="w-6  ml-2 text-2xl "
              />
        </h1>
        
      
              </div>
        <ul className="mb-6 flex gap-2.5">
          {RISK_LEVELS.map(({ id, title,koreanTitle }) => (
            <li
              key={id}
              className={`py-2 px-3 rounded-[36px] text-sm text-center cursor-pointer h-9 ${
                riskLevelType === title
                  ? "bg-black text-white"
                  : `border border-gray-200 ${COLORS[title]} text-gray-400`
              }`}
              onClick={() => setRiskLevelType(title)}
            >
              <h2 className={`h-5 ${riskLevelType != title ? COLORS[title]:""} `}>{router.locale =="ko" ? koreanTitle: title}</h2>
            </li>
          ))}
        </ul>

                    {riskLevelData&& !riskLevelValid ? 
                    <Carousels2 riskLevelData={riskLevelData}/>
          : 
