...
const Header = ({carouselData,carouselType,setCarouselType,carouselValid}:Props) => {
  //@ts-ignore
  ...
  return (
    <main  className="pt-[90px]  relative h-[415px] min-w-[1024px]  w-full  bg-backgroundColor bg-cover bg-no-repeat bg-center">
      ...
      <div className={'max-w-1320 w-full mx-auto flex flex-col justify-center items-cener  '}>
      <div className={'mb-10 flex gap-20 justify-between items-center ml-[60px] mr-[60px]'}>
      <h1 className=" text-center  text-[27px] xl:text-[40px] text-white">
        {t('headerTitle')}
      </h1>
      <button onClick={buttonHandler} className={' flex justify-center items-center  w-[350px] ml-10  py-5 px-10 mr-2  text-xl font-extrabold text-[#0198FF] shadow-md focus:outline-none bg-white rounded-full border-2  border-gray-300  hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700'}>{t('headerButton')}
      <Image
                src={arrow}
                alt=""
      ...
      <ul className="flex  gap-2.5 mb-10 justify-start  ml-[60px]">
        {MENUS.map(({ id, title,koreanTitle }:{id:number; title:string; koreanTitle:string}) => (
          <li
            key={id}
            className={`py-2 px-3 rounded-[36px] min-w-[46px] text-center cursor-pointer hover:bg-[#F3F4F6] hover:text-gray-400
            ${carouselType === title ? "bg-black text-white" : "bg-white text-gray-400"}`}
            onClick={() => setCarouselType(title)}
          >
            <h5 className="h-5">{router.locale =="ko" ? koreanTitle: title}</h5>
          </li>
        ))}
      </ul>
      ...
      <Carousels carouselData={carouselData}/> :   <div role="status" className={'py-20 justify-center items-center'} >
