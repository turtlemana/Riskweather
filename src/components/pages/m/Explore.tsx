import {KeyboardEventHandler ,useState,useEffect,useCallback,useMemo } from "react";
...
const Explore = () => {
  ...
  const {state,dispatch}: {state: State, dispatch: React.Dispatch<Action>} = useExploreState();
  ...
  useEffect(() => {
    dispatch({ type: 'SET_CURRENCY', payload: router.locale === 'ko' ? 'KRW' : 'USD' });
  
    if (router?.query?.type && router?.query?.type === 'Crypto') {
      setTmpState((prevState) => ({
        ...prevState,
        tmpType: router?.query?.type as string,
      }));
      dispatch({ type: 'SET_TYPE', payload: router.query.type });
    } else if (router?.query?.type && LOCATIONS.some((location) => location.title === router.query.type)) {
      setTmpState((prevState) => ({
        ...prevState,
        tmpLoc: router?.query?.type as string,
      }));
      //@ts-ignore
      dispatch({ type: 'SET_LOC', payload: router.query.type });
    } else if (router?.query?.type && RISKS.some((risk) => risk.title === router.query.type)) {
      setTmpState((prevState) => ({
        ...prevState,
        tmpRiskLv: router?.query?.type as string,
      }));
      //@ts-ignore
...
setIsFilterActive(false);
  ...
  const handleKeyUp : KeyboardEventHandler<HTMLInputElement> =event => {
    if (event.key === "Enter" || event.keyCode === 13) { // Enter 터치 이벤트 감지
  ...
  return (
    <main className="px-5 py-6 bg-white min-w-[360px] ">
      ...
      <p className="mr-3 text-xs text-gray-400 mb-5 ">{assetList.length>0 && !isValidating ? udtDate : ""}</p>
      ...
      <section className="flex gap-3 mb-4">
        <article className="w-[242px] h-10 p-3 flex items-center border border-gray-200 rounded-20 hover:border-[#4B5563]">
            ...
            <Image loading="eager" src={mobileSearch} alt="search" className="mr-2 w-4 h-4" />
        ...
        >
          ...
          <p className="mt-px text-gray-600 text-[10px] truncate w-[30px] ">{t("filterButton")}</p>
        </button>
      </section>
      {isFilterActive && (
        <div className="fixed z-30 bg-white w-full h-full top-16 left-0 overflow-auto customScrollBar pb-[140px]">
          ...
          <section className="py-7 pb-3">
            <article className="mb-7">
              <h6 className="px-5 text-sm text-[#111111] mb-3">{t("typeTitle")}</h6>
              <ul className="px-5 flex gap-[10px]">
                {TYPES.map(({ id, title,koreanTitle }) => (
                  ...
                  >                  
                    <h6
                      className={`text-sm ${
                        tmpState.tmpType == title ? "text-[#0198FF]" : "text-gray-500"
                      }`}
                    >
            ...
            <article className="mb-7 px-5">
              <h6 className="text-sm text-[#111111] mb-3">{t("risklevelTitle")}</h6>
              <ul className="flex gap-[10px] flex-wrap">
                {RISKS.map(({ id, title,koreanTitle }) => (
                  ...
                  >
                    <h6
                      className={`text-sm ${
                        tmpState.tmpRiskLv === title ? "text-[#0198FF]" : "text-gray-500"
                      }`}
                    >
                  {router.locale=="ko" ? koreanTitle : title}
                    </h6>
            ...
            <Select  defaultData={tmpState.tmpCurrency}  data={CURRENCIES} title={"Currency"} koreanTitle={"통화"} selectHandler={currencyHandler} />

            <div className="my-7 h-[6px] bg-gray-50 w-full" />
            <Select  defaultData={tmpState.tmpWeather}  data={WEATHERS} title={"Weather"} koreanTitle={"날씨"} selectHandler={weatherHandler} />

            <Select tmpType={tmpState.tmpType}  defaultData={tmpState.tmpLoc} data={LOCATIONS} title={"Location"} koreanTitle={"국가"} selectHandler={locHandler} />
        <Select tmpType={tmpState.tmpType} defaultData={tmpState.tmpExchg} data={EXCHAGNES} title={"Exchange"} koreanTitle={"거래소"}  selectHandler={exchgHandler} />
        <Select tmpType={tmpState.tmpType}  defaultData={tmpState.tmpSect} data={SECTROS} title={"Sector"} koreanTitle={"분야"} selectHandler={sectHandler} />
    ...
    <div role="status" className={'flex py-20 justify-center items-center'} >
      ...
      assetList.map((asset:AssetInfo, i:number) => (
        ...
        >
          ...
          >
          <Image loading="eager" unoptimized  width={50} height={50} quality={100} onError={(event)=>handleImageError(event,asset['HR_ITEM_NM'])} src={`/images/logos/${asset['ITEM_CD_DL']}.png` || ""} alt="" className="h-10 w-10 mr-2"/> 
            <div className="flex-1">
              <h6 className={'w-[150px] truncate'} title={`${router.locale=="ko" ? asset['ITEM_KR_NM'] : asset['ITEM_ENG_NM']}`}>{router.locale=="ko" ? asset['ITEM_KR_NM'] : asset['ITEM_ENG_NM']}</h6>
              <p className="text-gray-500 text-xs">{asset['ITEM_CD_DL']}</p>
            </div>
            <div className={`py-1 px-3 rounded-20 ${COLORS[asset['CVaR_LV']]} `}>
              <h6 className="text-xs">{router.locale=="ko" ? asset['CVaR_LV_KR'] : asset['CVaR_LV']}</h6>
            </div>
            <Image
              src={arrow}
              alt=""
          ...
          {isOpenDetail === i && (
            <article className="bg-gray-50 p-4 rounded-[8px] text-sm font-medium" onClick={()=>{router.push({pathname:`/detail/${asset['ITEM_CD_DL']}`})}}>
              <div className="flex mb-3">
                <p className="text-xs text-gray-400 flex-1">{t("tableLoss")}</p>
                <p className="text-[#DF1525] font-semibold">-{asset['EXP_CVaRNTS'].toFixed(2)}%</p>
              </div>
              <div className="flex mb-3">
                <p className="text-xs text-gray-400 flex-1">{t("tablePrice")}</p>
                <p className="text-gray-700 font-semibold">{asset['CAT'] =="Index"? "" : state.currency==="KRW" ? "￦" : "$"}{" "}{state.currency==="KRW" ? asset['ADJ_CLOSE_KRW'].toLocaleString('en-us',{minimumFractionDigits: 0, maximumFractionDigits: 0}):  asset['ADJ_CLOSE_USD'].toLocaleString('en-us',{minimumFractionDigits: 2, maximumFractionDigits: 2})}</p>
              </div>
              <div className="flex mb-3">
                <p className="text-xs text-gray-400 flex-1">{t("tablePriceChange")}</p>
                <p className={`font-semibold ${asset['ADJ_CLOSE_CHG']>0 ? "text-[#3CB043]" : "text-[#DF1525]"}`}>{asset['ADJ_CLOSE_CHG'].toFixed(2)}%</p>
              </div>
              <div className="flex mb-3 items-center">
                <p className="text-xs text-gray-400 flex-1">{t("tableRisk")}</p>
                <MiniChart chartData={JSON.parse(asset['CHART'])}/>

              </div>
              <div className="flex items-center">
                <p className="text-xs text-gray-400 flex-1">{t("tableWeather")}</p>
                <Image src={`/images/weather/${asset['WTHR_ENG_NM']}.svg`} width={0} height={0} alt="" className="w-6 h-6" />
                <p className="text-xs text-gray-600 ml-2">{router.locale=="ko" ? asset['WTHR_KR_DL'] : asset['WTHR_ENG_DL']}</p>
              </div>
            </article>
          )}
        </section>
