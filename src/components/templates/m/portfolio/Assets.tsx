    ...
    <main className="pt-7 pb-2">
      ...
      <section className="bg-white px-5 ">
              ...
              <div key={i}>
                <article className="flex justify-between items-center py-4">
                  ...
                  <Image loading="eager" unoptimized  width={10} height={10} quality={100} onError={handleImageError} src={`/images/logos/${ticker}.png` || ""} alt="" className="h-10 w-10 mr-3 "/>                       <div>
                      <h6 className={'w-[150px] truncate'}>{router.locale=="ko" ? krName : name}</h6>
                      <p className="text-gray-500 text-xs">{ticker}</p>
                    </div>
                  </div>
                  <div className="text-right text-[#111111] text-xs font-medium">
                    <h6 className="text-sm mb-1">  {currencySelect===1 ? "￦" +" " + krPrice?.toLocaleString('en-US',{minimumFractionDigits: 0, maximumFractionDigits: 0}) : "$" + " " + price?.toLocaleString('en-US',{minimumFractionDigits: 2, maximumFractionDigits: 2})}</h6>
                    <p className="text-gray-700">{quantity.toLocaleString()}</p>
                  </div>
                </article>
                {array.length - 1 !== i && (
        ...
        : (
                    ...
                    <p className="  font-semibold text-sm text-[#111111]">{t("noAssets")}</p>
                    <p className="pb-[30px]  font-semibold text-sm text-[#111111]">{t("noAssets2")}</p>

                  <button           
                  onClick={handleOpen}
                  type="button" 
                  className="w-[150px] h-[50px]   text-white bg-gradient-to-br from-purple-600 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-bold rounded-xl shadow-sm text-md px-5 py-2.5 text-center mr-2 mb-2">{t('assetAdd')}</button>

                  </div>
                  </div>
        )}
                              <p className={'text-end text-gray-400 text-sm mt-5'}> {ptfData.length>0 && !isValidating ? udtDate : ''}</p>
