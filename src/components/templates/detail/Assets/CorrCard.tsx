...
const CorrCard = ({EXP_CVaRNTS,exchg,krName,riskDescriptionEn,riskDescriptionKr, name,ticker,riskLevel,maxLoss,weather, weatherExplain, koreanRiskLevel, koreanWeatherExplain }: Props) => {
  ...
  return (
    <main className="mr-6 p-5 cursor-pointer rounded-20 min-w-[296px] shadow-[0_0_12px_0_rgba(121,120,132,0.15)] overflow-auto" onClick={()=>{router.push({pathname:`/detail/${ticker}`})}}>
      ...
      <Image loading="eager" unoptimized  width={50} height={50} quality={100} onError={(event)=>handleImageError(event,exchg)} src={`/images/logos/${ticker}.png` || ""} alt="" className="h-10 w-10 mr-3 "/>        <article className="flex-1">
          <h6 className="mb-1 w-[100px] truncate" title={router.locale=="ko" ? krName : name}>{router.locale=="ko" ? krName : name}</h6>
          <p className="text-[12px] text-gray-500">{ticker}</p>
        </article>
        <div 
        data-tooltip-id="riskLevel"
        data-tooltip-content={router.locale=="ko" ? riskDescriptionKr : riskDescriptionEn}
