...
import {useEffect,useMemo,Dispatch, KeyboardEventHandler } from "react";
...
const SearchModal = ({search,onSearch,searchList,setIsOpenSearchModal}:Props) => {
  ...
  useEffect(()=>{  
    router.events.on('routeChangeStart', ()=>{setIsOpenSearchModal(false)})
},[router.events])
  ...
  const handleKeyUp:KeyboardEventHandler<HTMLInputElement> = (event) => {
    if (event.key === "Enter" || event.keyCode === 13) { // Enter 터치 이벤트 감지
      ...
      if (resultList.length >= 1) {
        const firstItem = resultList[0]; // 첫 번째 항목
        //@ts-ignore
    ...
    <main className="fixed top-16 left-0 w-full h-full bg-white z-20 py-6 px-5 overflow-auto pb-20">
      ...
      <section>
        <article className="flex mb-5">
          <h6 className="text-[#0148FF] mr-1">{resultList?.length}</h6>
          <h6 className="text-[#111111]">{router.locale=="ko" ? "결과" : "Result"}</h6>
        </article>
        <article>
          {resultList?.map(
            ({HR_ITEM_NM, ITEM_ENG_NM,ITEM_CD_DL,CVaR_LV,WTHR_ENG_NM,CVaR_LV_KR, ITEM_KR_NM }:ResultItem,i:number) => (
              <div
                className="border-b border-gray-100 flex items-center"
                key={i}
                onClick={()=>{router.push({pathname:`/detail/${ITEM_CD_DL}`})}}
              >
                <div className="flex items-center py-3 flex-1">
                <Image loading="eager" unoptimized  width={50} height={50} quality={100} onError={(event)=>handleImageError(event,HR_ITEM_NM)} src={`/images/logos/${ITEM_CD_DL}.png` || ""} alt="" className="h-10 w-10 mr-3"/>                  <div>
                   
                    <h6 className="text-sm w-[150px] truncate">{router.locale=="ko" ? ITEM_KR_NM : ITEM_ENG_NM}</h6>
                ...
                <Image onError={handleWeatherImageError} loading="eager" src={`/images/weather/${WTHR_ENG_NM}.svg`} alt="" width={25} height={25} className="ml-3" />
              </div>
            )
          )}
        </article>
