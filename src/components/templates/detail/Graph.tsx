  ...
  const chartData = useMemo(() => {
    ...
    if (router.locale == "ko") {
      completeData = orderedData.map((data: DetailChartInterface) => {
        return {"날짜": data.date, "리스크(CVaR)": data.cvar, "리스크(RW Index)": data.rwi, "리스크(EW Index)": data.ewi, "가격": data.price};
      });
    } else {
      completeData = orderedData.map((data: DetailChartInterface) => {
        return {"Date": data.date, "Risk(CVaR)": data.cvar, "Risk(RW Index)": data.rwi, "Risk(EW Index)": data.ewi, "Price": data.price};
      });
    }
    ...
    <main className="max-w-1320 min-w-[1024px] mx-auto p-8 rounded-20 bg-white my-7">
      <section className="flex mb-5 relative">
        ...
        >
          ...
          />
          <h6 className="h-3 text-[10px] w-[48px] text-center z-10">RW Index</h6>
        ...
        >
          ...
          <p className="text-[#0198FF] mb-1 text-xs h-3">{t('chartDownload')}</p>
      ...
      <ul className="flex gap-3  mb-[42px]  px-1 justify-start text-center items-center ">
          <li className="flex items-center  ">
            <p className="text-sm font-bold">{t('chartHighestPrice')}{": "}<span className={'font-medium'}>{highestPrice?.price.toLocaleString('en-us',{minimumFractionDigits: 0, maximumFractionDigits: 2})}{" "}{cat=="Index" ? "" : curr}</span></p>
          </li>
          <div className="w-px h-4 bg-gray-300 mb-1 mx-1" />
          <li className="flex items-center gap-2 ml-2">
            <p className="text-sm font-bold">{t('chartLowestPrice')}{": "}<span className={'font-medium'}>{lowestPrice?.price.toLocaleString('en-us',{minimumFractionDigits: 0, maximumFractionDigits: 2})}{" "}{cat=="Index" ? "" : curr}</span></p>
          </li>
          </ul>
