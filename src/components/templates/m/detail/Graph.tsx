...
const Graph = ({detailChart,curr,cat}:Props) => {
  ...
  const chartData = useMemo(() => {
    ...
    if (router.locale == "ko") {
      completeData = orderedData.map((data: DetailChartInterface) => {
        return {"날짜": data.date, "CVaR": data.cvar, "RW Index": data.rwi, "EW Index": data.ewi, "가격": data.price};
      });
    } else {
      completeData = orderedData.map((data: DetailChartInterface) => {
        return {"Date": data.date, "CVaR": data.cvar, "RW Index": data.rwi, "EW Index": data.ewi, "Price": data.price};
      });
    }
  ...
  const [highestPrice, setHighestPrice] = useState(tmpOriginData.reduce((prev:DetailChartInterface,current:DetailChartInterface)=>{return (prev.price>current.price) ? prev:current}))
  const [lowestPrice, setLowestPrice] = useState(tmpOriginData.reduce((prev:DetailChartInterface,current:DetailChartInterface)=>{return (prev.price>current.price) ? current:prev}))
  ...
  useEffect(()=>{ 
    ...
    if (selected=="All"){
        ...
        setHighestPrice(tmpOriginData.reduce((prev:DetailChartInterface,current:DetailChartInterface)=>{return (prev.price>current.price) ? prev:current}))
        setLowestPrice(tmpOriginData.reduce((prev:DetailChartInterface,current:DetailChartInterface)=>{return (prev.price>current.price) ? current:prev}))
        
    }else if (selected=="7D"){
        tmpData=chartData.slice(-7)
        tmpOriginData=detailChartData.slice(-7)
        ...
        tmpOriginData=detailChartData.slice(-15)
        ...
        tmpOriginData=detailChartData.slice(-30)
        ...
        tmpOriginData=detailChartData.slice(-90)
        ...
        tmpOriginData=detailChartData.slice(-180)
        ...
        tmpOriginData=detailChartData.slice(-365)
    ...
    <main className="bg-white pb-7">
      <section className="w-full mb-3">
        <ul className="flex gap-2 bg-gray-50 px-5 py-3 overflow-scroll customScrollBar items-center justify-center">
          {MENU_LIST.map(({ id, title,koreanTitle }) => (
            ...
            >
              <h6
                className={`${router.locale=="ko" ? "text-[10px] " : "text-xs "} pt-px ${
      ...
      <article
        ...
        >
          ...
          />
          <h6 className="h-3 text-[10px] w-[58px] text-center z-20">RW Index</h6>
        ...
        <button
          className="flex gap-1 px-2.5 py-1.5 border border-solid border-[#0198ff] rounded-20 bg-blue-rgba hover:bg-[#CCEAFF]"
        >
          <p className="text-[#0198FF] text-xs h-3 mb-1">{t('chartDownload')}</p>
      ...
      <section className="w-full h-[260px]"><MobileDetailChart chartSelect={chartSelect} chartData={chartSlice}/>
     ...
     <div className="flex  mt-10 justify-center text-center items-center">
          <li className="flex items-center flex-row  ">
            ...
            <p className="text-xs ">{highestPrice?.price.toLocaleString('en-us',{minimumFractionDigits: 0, maximumFractionDigits: 2})}{" "}{cat=="Index" ? "" : curr}</p>
          </li>
          <div className="ml-3 mr-3 w-px h-[14px] bg-gray-300" />

          <li className="flex items-center flex-row">
            <p className="text-xs font-bold">{t('chartLowestPrice')} :{" "} </p>
            <p className="text-xs ">{lowestPrice?.price.toLocaleString('en-us',{minimumFractionDigits: 0, maximumFractionDigits: 2})}{" "}{cat=="Index" ? "" : curr}</p>
          </li>
          </div>
      </section>
    </main>
