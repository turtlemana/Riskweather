...
export default function Main({indexData}:Props) {
  ...
  const [carouselType,setCarouselType]=useState(router.locale=="ko" ? "Korean Stock" : "Crypto")
  const [trendingType,setTrendingType]=useState(router.locale=="ko" ? "Korea (South)" : "Crypto")
  const [riskLevelType,setRiskLevelType]=useState("Very high")
  const [mapSelect, setMapSelect]=useState("")


  const {data:carouselDt, isValidating:carouselValid}=useSWR(`/api/carousel?type=${carouselType}`,fetcher)
  const {data:nowTrendingDt, isValidating:nowTrendingValid}=useSWR(`/api/nowTrending?type=${trendingType}`,fetcher)
  const {data:riskLevelDt, isValidating:riskLevelValid}=useSWR(`/api/riskLevel?level=${riskLevelType}`,fetcher)
  const {data:locDt,isValidating:locValid}=useSWR(mapSelect ? `/api/mapResult?mapSelect=${mapSelect}`:null,fetcher)
