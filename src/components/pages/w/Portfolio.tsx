...
const Portfolio = ({session}:{session:Session}) => {
  ...
  // const {data, isValidating:searchValid}=useSWR(search ? `/api/portSearch?search=${search}` : `/api/defaultAssets`,fetcher)
  ...
  return (
    <main className=" py-10 pb-[96px]">
      ...
      <section className="flex gap-5 max-w-1320 min-w-[1024px]  mb-7 mx-auto">
        ...
        ptfData={ptfData} userProfile={userProfile} mutate={mutate}/>
        <Assets
        currencySelect={currencySelect}
      ...
      <section className="flex max-w-1320 min-w-[1024px] mx-auto bg-white rounded-20 py-6 px-8 items-center">
        <h1 className="text-2xl text-gray-900 flex-1">{t("portfolioCustomerService")}</h1>
        <button  onClick={() => {
