...
const Profile = ({session, setIsOpenProfileModal, setIsOpenDeleteModal,userProfile,isValidating,mutate  }: Props) => {
  ...
  }).then((res)=>{if(res.ok){toast(router.locale=="ko" ? "프로필 변경이 완료됐습니다" :"Successfully changed your profile",{ hideProgressBar: true, autoClose: 2000, type: "success" });mutate()}else{toast(router.locale=="ko" ? "네트워크 에러가 발생했습니다" : "Fetch Error",{ hideProgressBar: true, autoClose: 2000, type: "error" })}})
    setIsOpenProfileModal(false)
    mutate()
    // sessionUpdate()
  }
    ...
    <main className="z-20 fixed bg-white p-6 rounded-20 top-1/2 left-1/2 translate-x-half translate-y-half border w-full max-w-[332px]">
      ...
      <section>
        ...
        <article className="mb-6">
          <p className="text-sm text-gray-500 font-medium mb-3">{t('profileName')}</p>
          <input
          type="text"
