...
const handler=async(req:NextApiRequest,res:NextApiResponse)=>{
    ...
    try{
        ...
        const result:CarouselData[]=await executeQuery({
            ...
            ${type=="All"||!type ? `AND al.ITEM_CD_DL IN ('BTC-USD','ETH-USD','AAPL','TSLA','MSFT','2330.TW','005930','XRP-USD','ACWI','^IXIC','NVDA')`:type==="Korean Stock" ? `AND al.loc="Korea (South)" `: `AND al.CAT = "${type}"` }  
