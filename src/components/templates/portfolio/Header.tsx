...
const Header = ({ setIsOpenProfileModal,userProfile }: Props) => {
  ...
  return (
    <main className="py-8 px-10 bg-white max-w-1320 min-w-[1024px] rounded-20 flex gap-[52px] mx-auto mb-10">
      ...
      width={10} height={10} unoptimized src={userProfile?.profileImage ? 
        userProfile?.profileImage 
        : '/images/logos/errorLogo.png'}           referrerPolicy="no-referrer"
                loading="eager" alt="" className="w-[100px] h-[100px]"  />
          <Image
            src={pencil}
            alt=""
        ...
        <h6 className="text-[28px] text-[#111111] mt-4 w-[150px] truncate">{userProfile?.name}</h6>
      </section>
      <section className="max-w-[283px] w-full">
        ...
        <article className="flex gap-4 items-center border border-gray-200 rounded-20 py-2.5 px-3">
          <Image data-tooltip-id="traitExplain" width={50} height={50} src={userProfile?.interestedResult ? `/images/traits/${userProfile?.interestedResult}.svg` : '/images/logos/errorLogo.png'} alt="" />
          <div>
            <h5 data-tooltip-id="traitExplain" className="text-[#0198FF] text-sm mb-1">
              {userProfile?.interestedResult ? 
              router.locale=="ko" ? 
              userProfile?.interestedResult == "Aggressive" ? "공격적인" :
              userProfile?.interestedResult == "Moderate" ? "중립적인" :
              userProfile?.interestedResult == "Moderately conservative" ? "약간 보수적인" :
              userProfile?.interestedResult == "Conservative" ? "보수적인" :
              "위험 성향을 측정하세요" : userProfile?.interestedResult
              
              :  router.locale=="ko" ? "위험 성향을 측정하세요" : "Undefined Result"}
            </h5>
      ...
      <section className="w-full max-w-[586px]">
        ...
        <article className="flex gap-2 flex-wrap max-h-[80px] overflow-auto slim-scroll">
          {userProfile?.interestedAssets?.map(({ name, ticker,krName }:{name:string; ticker:string; krName:string;}, i:number) => (
            <div
            ...
            >
      <Image loading="eager" unoptimized  width={50} height={50} quality={100} onError={handleImageError} src={`/images/logos/${ticker}.png` || ""} alt="" className="h-6 w-6 "/>                  
      <p className="text-[#111111] text-xs font-medium mt-px w-[80px] truncate" title={router.locale=="ko" ? krName : name}>{router.locale=="ko" ? krName : name}</p>
            </div>
          ))}
        </article>
      </section>
