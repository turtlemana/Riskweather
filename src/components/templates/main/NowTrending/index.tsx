    ...
    <main className="mb-7 px-5 min-w-[1024px]">
      <div className=" max-w-1320 w-full mx-auto bg-white rounded-20 overflow-hidden">
        <article className="flex justify-between p-8 mb-2">
          <h1 className="text-[28px] text-[#111111]">{t('nowTrendingTitle')}</h1>
          <div onClick={()=>{ dispatch({ type: 'RESET_VALUE' }); router.push({pathname:`/${router.locale}/explore`, query:{type:trendingType}})}} className={'flex items-center cursor-pointer'}>
              <button type="button" className="flex items-center py-3 px-4  justify-center  text-sm font-bold text-gray-900 focus:outline-none bg-white rounded-full border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">{t('toExplore')}
              <Image
                src={arrow}
                alt=""
                className="w-4 ml-2  "
          ...
          {TRENDING_MENUS.map(({ id, title,koreanTitle }) => (
            ...
            >
              {router.locale=="ko" ? koreanTitle : title}
                    ...
                    <p className="text-sm text-[#6B7280] mr-10"> {nowTrendingData.length>0 && !nowTrendingValid ? udtDate : ''}</p>
        ...
        <table className="w-full">
          ...
          <tbody>
            ...
            {nowTrendingData&& !nowTrendingValid ? nowTrendingData.map((asset:NowTrendingData,i:number)=><Items exchg={asset['HR_ITEM_NM']} krName={asset['ITEM_KR_NM']} name={asset['ITEM_ENG_NM']} ticker={asset['ITEM_CD_DL']} key={i} riskLevel={asset['CVaR_LV']} koreanRiskLevel={asset['CVaR_LV_KR']} maxLoss={asset['CVaRNTS']} price={asset['ADJ_CLOSE']} currency={asset['CURR']} priceChange={""} chartData={JSON.parse(asset['CHART'])} weather={asset['WTHR_ENG_NM']} weatherExplain={asset['WTHR_ENG_DL']} koreanWeatherExplain={asset['WTHR_KR_DL']} riskDescriptionEn={asset['LV_DSCP_ENG']} riskDescriptionKr={asset['LV_DSCP_KR']} EXP_CVaRNTS={asset['EXP_CVaRNTS']} />) :
