      ...
      <div className="px-5 mx-auto bg-white rounded-20 overflow-auto">
        <div className={"flex justify-between"}>
          <h1
            ...
            className="flex items-center text-[28px] text-[#111111] mb-5"
          >
            {t("riskLevelTitle")}
            <Image src={blackArrow} alt="" className="w-6  ml-2 text-2xl " />
          </h1>
        </div>
        <ul className="mb-6 flex gap-2.5">
          {RISK_LEVELS.map(({ id, title, koreanTitle }) => (
            <li
              key={id}
              className={`py-2 px-3 rounded-[36px] text-sm text-center cursor-pointer  ${
                riskLevelType === title
                  ? "bg-black text-white"
            ...
            >
              <h2
                className={` ${riskLevelType != title ? COLORS[title] : ""} `}
              >
