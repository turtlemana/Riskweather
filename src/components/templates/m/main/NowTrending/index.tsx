    ...
    <main className="mb-7 mt-[60px]">
      <div className="w-full mx-auto bg-white">
        <article className="flex justify-between mb-5 items-center px-5">
          <h1 className="flex items-center justify-center text-2xl text-[#111111]"  onClick={() => {
            dispatch({ type: 'RESET_VALUE' });
            router.push({ pathname: `/${router.locale}/explore`, query: { type: trendingType } });
          }}>{t("nowTrendingTitle")}
          <Image
                src={blackArrow}
                alt=""
          ...
          <p className="text-xs text-[#6B7280]">{nowTrendingData && !nowTrendingValid ? udtDate : ''}</p>
        </article>
        <article className="pl-5 mb-5 flex gap-6 border-b border-gray-200">
          {TRENDING_MENUS.map(({ id, title,mobileEngTitle, koreanTitle }) => (
            <h2
              ...
              onClick={() => {setIndex(id);setTrendingType(title);setList(1)}}
            >
              {router.locale =="ko" ? koreanTitle: mobileEngTitle}
                    ...
                    {nowTrendingData&& !nowTrendingValid ? array.map((asset:NowTrendingData,i:number)=><Items EXP_CVaRNTS={asset['EXP_CVaRNTS']} exchg={asset['HR_ITEM_NM']} name={asset['ITEM_ENG_NM']} krName={asset['ITEM_KR_NM']} ticker={asset['ITEM_CD_DL']} key={i} riskLevel={asset['CVaR_LV']} koreanRiskLevel={asset['CVaR_LV_KR']} koreanWeatherExplain={asset['WTHR_KR_DL']}  maxLoss={asset['CVaRNTS']} price={asset['ADJ_CLOSE']} currency={asset['CURR']} priceChange={""} chartData={JSON.parse(asset['CHART'])} weather={asset['WTHR_ENG_NM']} weatherExplain={asset['WTHR_ENG_DL']} id={i}/>) :
                    <div role="status" className={'py-20 flex items-center justify-center '} >
        ...
        {!isLast ? (
          <div className="px-5">
            ...
            >
              <p className="text-[#0198FF] text-sm font-medium">{t("nowTrendingButton1")}</p>
        ...
        <button
          ...
          >
          <p className="text-[#0198FF] text-sm font-medium">{t("toExplore")}</p>
          <SlArrowRight className={' ml-1.5 text-[#0198FF] text-[10px] font-bold'}/>
          {/* <Image src={arrow} alt="" className="mb-1 ml-1" /> */}
        </button>
      </div>}
      </div>
