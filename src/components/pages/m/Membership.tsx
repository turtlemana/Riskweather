...
const Membership = ({session}:Props) => {
  ...
  const onPaymentButtonClick = (koreanPrice:string) => {
    ...
    async function callback(response:PaymentResponse) {
      ...
      if (success) {
        toast(router.locale=="ko" ? "결제에 성공하셨습니다" :"Payment has successfully completed",{ hideProgressBar: true, autoClose: 2000, type: "success" });
        let enteredInput={
          membership:1
        }
      ...
      }).then((res)=>{if(res.ok){toast(router.locale=="ko" ? "맴버십 단계가 스탠다드로 설정됐습니다" :"Your Membership is now registered as Standard",{ hideProgressBar: true, autoClose: 2000, type: "success" });}else{toast(router.locale=="ko" ? "네트워크 에러가 발생했습니다" : "Fetch Error",{ hideProgressBar: true, autoClose: 2000, type: "error" })}})
        // sessionUpdate()
    } else{
      toast(router.locale=="ko" ? "결제에 실패했습니다" :"Payment Error",{ hideProgressBar: true, autoClose: 2000, type: "error" });
  ...
  return (
    <main className="min-w-[360px] pt-12 pb-[100px] px-5 relative">
      ...
      <section className="mb-12 flex justify-center gap-2.5 text-2xl">
        ...
        <h6 className="text-[#0198FF] underline decoration-2">{t('membershipTitle2')}</h6>
      </section>
      <ul>
        {MEMBERSHIPS.map(({ id, title, price, request,koreanTitle, koreanPrice,showingKrPrice }) => (
          <li
            key={id}
            className="py-10 px-8 mb-6 rounded-20 bg-white border border-gray-200 w-full"
          >
            <h6 className="text-2xl text-gray-600 mb-3">{router.locale=="ko" ? koreanTitle : title}</h6>
            <h1 className="text-xl text-[#111111] mb-10">{router.locale=="ko" ? showingKrPrice : price}</h1>
            <div className="flex gap-2.5 items-end mb-10">
              <h2 className="text-[#0198FF] text-2xl">{request}</h2>
              <p className="font-medium text-sm text-gray-800 mb-1">
              {title=="Basic" ? t('membershipRequest') : t('membershipRequest2')}
              </p>
            </div>
            <button disabled={title==="Basic" ? true : false} onClick={()=>onPaymentButtonClick(koreanPrice)}  className="bg-[#0198FF] w-full py-[10px] rounded-[60px] hover:bg-[#0085E6] disabled:bg-[#D1D5DB]">
              <h3 className="text-white text-sm">{router.locale=="ko" ? showingKrPrice : price}</h3>
            </button>
          </li>
        ))}
      </ul>
