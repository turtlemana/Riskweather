...
const Assets = ({session,interestedList}:Props) => {
  ...
  const handleSelect = ({name,ticker,krName}:CoinSelect) => {
    if (selectCoin.some(asset=>asset['ticker']==ticker)) {
      const filtered = selectCoin.filter((asset:CoinSelect) => asset.ticker !== ticker);
      return setSelectCoin(filtered);
    }
    if (selectCoin.length>=10){return}
  ...
  toast(router.locale=="ko" ? "위험 성향 측정이 완료됐습니다" :"Calculation completed!",{ hideProgressBar: true, autoClose: 2000, type: "success" })
  const data=await fetch(`/api/auth/addUserInfo?session=${session.user.email}`,{
    method:"POST",
    headers:{"content-type":"application/json"},
    body:JSON.stringify({enteredInput})
}).then((res)=>{if(res.ok){sessionUpdate();setIsModalOpen(true);}else{toast(router.locale=="ko" ? "네트워크 에러가 발생했습니다" :"Fetch Error",{ hideProgressBar: true, autoClose: 2000, type: "error" })}})
  
} catch (err){
  setIsLoading(false);
    toast(router.locale=="ko" ? "네트워크 에러가 발생했습니다" :"Fetch Error",{ hideProgressBar: true, autoClose: 2000, type: "error" })
  }
  ...
  const data=await fetch(`/api/auth/addUserInfo?session=${session.user.email}`,{
      ...
      headers:{"content-type":"application/json"},
      body:JSON.stringify({enteredInput})
  }).then((res)=>{if(res.ok){sessionUpdate();setIsSkipModalOpen(true)}else{toast(router.locale=="ko" ? "네트워크 에러가 발생했습니다" :"Fetch Error",{ hideProgressBar: true, autoClose: 2000, type: "error" })}})
  ...
  return (
    <main
      className="min-w-[360px] relative py-5 bg-white text-center"
    >
      <div
        className="flex cursor-pointer mb-3 justify-end text-xs items-center"
        onClick={()=>session?.user?.accessLevel==1 ? bankHandler("skip") : skipHandler()}
      >
        <p>
       {t('assetSkip')}</p>
      ...
      <div className={'text-start  rounded-md border p-1.5 mb-8 bg-backgroundLearn w-full h-[150px] z-10 text-white bg-cover bg-no-repeat bg-center'}>
      ...
      {interestedList.map(({HR_ITEM_NM, ITEM_ENG_NM,ITEM_CD_DL,ITEM_KR_NM  }:Asset,i:number) => (
          <article
            className="cursor-pointer max-w-[104px] mx-auto"
            key={i}
            onClick={() => {
              if (isSkipModalOpen || isModalOpen || isBankModalOpen ||isLoading) {
                return }
              handleSelect({name:ITEM_ENG_NM,ticker:ITEM_CD_DL,krName:ITEM_KR_NM})}}
          >
            <div className="relative mb-3 w-[60px] h-[60px] mx-auto">
            <Image loading="eager" unoptimized  width={0} height={0} quality={100} onError={(event)=>handleImageError(event,HR_ITEM_NM)} src={`/images/logos/${ITEM_CD_DL}.png` || ""} alt="" className={`${selectCoin.some(asset=>asset['ticker']==ITEM_CD_DL) && "bg-light-gray-rgba z-10 bg-cover"} 
       w-full h-full `}/>    
      ...
      {selectCoin.length >=2 && (!isModalOpen && !isSkipModalOpen) ? (
          <button
            ...
            onClick={()=>session?.user?.accessLevel==1 ? bankHandler("submit") : submitHandler()}
          >
      ...
      {isLoading &&     
       ...
       <article className={'items-center w-[332px] mx-auto h-fit left-1/2 py-12   top-[220px]   text-[#111111] text-sm border bg-white   rounded-20  z-20 shadow-[0_0_12px_0_rgba(121,120,132,0.15)]'}>
     ...
     <div className={'flex items-center justify-center'}>
      ...
      {isModalOpen && (
        ...
        <article className="h-fit border w-[332px] mx-auto bg-white  py-12 rounded-20  top-[220px] left-1/2   z-20 text-sm  shadow-[0_0_12px_0_rgba(121,120,132,0.15)]">
          ...
          <button
                        onClick={()=>router.replace({pathname:`/${router.locale}/portfolio`})}
...
<article className="h-fit border w-[332px] mx-auto bg-white  py-12 rounded-20  top-[220px] left-1/2   z-20 text-sm  shadow-[0_0_12px_0_rgba(121,120,132,0.15)]">
  ...
  >
    <option disabled value="">{router.locale=="ko" ? "증권사 선택" : "Select securities company"}</option>
    <option value="KB증권">{router.locale=="ko" ? "KB증권" : "KB Securities"}</option>
    <option value="NH투자증권">{router.locale=="ko" ? "NH투자증권" : "NH Investment & Securities"}</option>
    <option value="삼성증권">{router.locale=="ko" ? "삼성증권" : "Samsung Securities"}</option>
    <option value="미래에셋증권">{router.locale=="ko" ? "미래에셋증권" : "Mirae Asset Securities"}</option>
    <option value="키움증권">{router.locale=="ko" ? "키움증권" : "Kiwoom Securities"}</option>
    <option value="토스증권">{router.locale=="ko" ? "토스증권" : "Toss Securities"}</option>
    <option value="카카오페이증권">{router.locale=="ko" ? "카카오페이증권" : "Kakaopay securities"}</option>
    <option value="한국투자증권">{router.locale=="ko" ? "한국투자증권" : "Korea Securities"}</option>
    <option value="하나증권">{router.locale=="ko" ? "하나증권" : "Hana Securities"}</option>
    <option value="기타">{router.locale=="ko" ? "기타" : "Others"}</option>
    {/* Add more options as needed */}
  </select>
  <div className={`flex items-center justify-center mb-5 ${router.locale==="ko" ? "" : "px-5"}`}>
    <input onChange={()=>setIsApproved(prev => !prev)} id="link-checkbox" type="checkbox" value="" className="ml-2 w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"/>
    <label htmlFor="link-checkbox" className="mr-3 text-[10px] text-light text-gray-400 font-medium  dark:text-gray-300">{t('agreements')} <Link target="_blank" href="/policy" className="text-blue-600 dark:text-blue-500 hover:underline">{t('agreements2')}</Link></label>
</div>
  <button
