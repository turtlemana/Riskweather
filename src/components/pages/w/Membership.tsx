  ...
  return (
    <main className="min-w-[1024px] pt-[108px] pb-[240px] px-5 bg-backgroundMembership bg-cover bg-no-repeat bg-center">
      ...
      <ul className="flex justify-center gap-[30px]">
        {MEMBERSHIPS.map(({ id, title, koreanTitle,price,showingKrPrice, koreanPrice, request }) => (
          <li
            key={id}
            className="py-[60px] px-12 rounded-20 bg-white border border-gray-200 w-full max-w-[440px]"
          >
            <h6 className="text-4xl text-gray-600 mb-5">{router.locale=="ko" ? koreanTitle : title}</h6>
            <h1 className="text-[28px] text-[#111111] mb-[56px]">{router.locale=="ko" ? showingKrPrice : price}</h1>
            ...
            <button disabled={title==="Basic" ? true : false} onClick={()=>onPaymentButtonClick(koreanPrice)} className="bg-[#0198FF] w-full py-3 rounded-[60px] hover:bg-[#0085E6] disabled:bg-[#D1D5DB]">
              <h3 className="text-white ">{router.locale=="ko" ? showingKrPrice : price}</h3>
            </button>
          </li>
        ))}
      </ul>
