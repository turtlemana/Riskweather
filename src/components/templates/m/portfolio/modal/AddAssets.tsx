...
const AddAssets = ({currencySelect, session, setIsOpenAddModal,userProfile,isValidating,mutate,search,setSearch,onSearch,searchList,searchValid,rowCount, page, setPage, limit, setLimit   }: Props) => {
  ...
  const submitHandler=async()=>{
    if (selectIndex.some(asset=>asset['quantity']<=0)) {toast(router.locale=="ko" ? "최소 1개 이상의 자산을 추가해야 합니다" :"your asset should be more than 0",{ hideProgressBar: true, autoClose: 2000, type: "error" }); return}
    let enteredInput={}
  ...
  try {
    ...
    toast(router.locale=="ko" ? "네트워크 에러가 발생했습니다" :"Fetch Error", {
      hideProgressBar: true,
      autoClose: 2000,
      type: "error",
    });
  ...
  return (
    ...
    <main className="z-30 fixed text-center bg-white border w-full h-full top-0 overflow-auto customScrollBar">
      ...
      <p className="text-sm text-[#6B7280] text-end mr-5 mt-2"> {searchList.length>0 && !isValidating ? udtDate : ''}</p>
      ...
      <section className="mx-5 mt-5 mb-6 h-10 py-2.5 px-4 flex items-center border border-solid border-gray-200 rounded-20 hover:border-[#4B5563]">
        ...
        <Image loading="eager" src={searchIcon} alt="search" className="mr-2 w-4 h-4" />

        <input placeholder={router.locale=="ko" ? "자산의 이름을 검색하세요" : "Search"} className="outline-none text-sm" value={search}
             onChange={onSearch} />
      </section>
    {!searchValid ?  <section className="px-5 mb-5">
        {searchList.map(({ADJ_CLOSE_KRW,HR_ITEM_NM,EXP_CVaRNTS,LOC,ITEM_KR_NM,ITEM_ENG_NM,ITEM_CD_DL,CVaR_LV,CVaRNTS,ADJ_CLOSE,WTHR_ENG_NM,WTHR_ENG_DL,ADJ_CLOSE_USD,WTHR_KR_DL, CVaR_LV_KR}:SearchList,i:number) => (
          <div key={i}>
            <article className="border-b border-gray-100 flex py-3 items-center justify-between">
              {selectIndex.some(asset=>asset['ticker']==ITEM_CD_DL) ? (
                <Image
                  ...
                  onTouchStart={() =>  handleSelect({name:ITEM_ENG_NM,ticker:ITEM_CD_DL,quantity:0, krName:ITEM_KR_NM})}
                />
              ) : (
                <Image
                  src={checkbox}
              ...
              <Image loading="eager" unoptimized  width={10} height={10} quality={100} onError={(event)=>handleImageError(event,HR_ITEM_NM)} src={`/images/logos/${ITEM_CD_DL}.png` || ""} alt="" className="h-10 w-10 mr-3 "/>   
                <div>
                  <h6 className={'truncate w-[130px]'}>{router.locale=="ko" ? ITEM_KR_NM : ITEM_ENG_NM}</h6>
                  <p className="text-gray-500 text-xs">{ITEM_CD_DL}</p>
                </div>
              </div>
              <div className={`py-1 px-3 rounded-20 ${COLORS[CVaR_LV]} mr-3`}>
            ...
            {isOpenDetail === i && (
              <article className="bg-gray-50 p-4 rounded-[8px] text-sm font-medium text-gray-700 text-left">
                <div className="flex mb-3">
                  <p className="text-xs text-gray-400  flex-1">{t('tableLoss')}</p>
                  <p className={'font-semibold text-[#DF1525]'}>-{EXP_CVaRNTS.toFixed(2)}%</p>
                </div>
                <div className="flex mb-3">
                  <p className="text-xs text-gray-400 flex-1">{t('tablePrice')}</p>
                ...
                <div className="flex items-center">
                  <p className="text-xs text-gray-400 flex-1">{t('tableWeather')}</p>
                  <Image src={`/images/weather/${WTHR_ENG_NM}.svg`} className={'h-6 w-auto'} width={0} height={0} alt="" />

                  <p className="text-xs text-gray-600 mt-1">{router.locale=="ko" ? WTHR_KR_DL : WTHR_ENG_DL}</p>
                </div>
              </article>
            )}
          </div>
      ...
      {userProfile?.portfolio ? (userProfile?.portfolio.length>0) ? (selectIndex.length>0) && (userProfile?.portfolio.length + selectIndex.length)>=1 ? (
          <button
          ...
          >
            {t('assetFinish')} ({userProfile?.portfolio?.length + selectIndex.length})
          </button>
        ) : 
        <div></div>
      //   <button
    ...
    >
     ...
     disabled={true}
     className=" sticky  h-fit bottom-[10px] bg-[#0198ff] text-white py-3 px-10 rounded-[60px] text-[16px] font-bold w-[236px] disabled:bg-gray-400"
     onClick={()=>{toast(router.locale=="ko" ? "2개 이상의 자산을 고르세요" :"You should select at least 2 assets",{ hideProgressBar: true, autoClose: 2000, type: "error" })}}
     >
       {t('assetFinish')} ({selectIndex.length})
</button>
...
<p className="text-gray-500 font-medium text-sm mr-[26px]">{router.locale=="ko" ? "이름" : "Name"}</p>
<p className=" text-gray-500 font-medium text-sm ml-[70px]">{router.locale=="ko" ? "수량" : "Quantity"}</p>
</div>
  {selectIndex.map(({ name, ticker,quantity,krName }, i) => (
    ...
    <div className="flex items-center py-3" key={i}>
      ...
      <Image loading="eager" unoptimized  width={50} height={50} quality={100} onError={handleWeatherImageError} src={`/images/logos/${ticker}.png` || ""} alt="" className="mr-3 h-6 w-6  "/>
        <div>
          <h6 className={'text-sm truncate w-[80px]'} >{router.locale=="ko" ? krName : name}</h6>
