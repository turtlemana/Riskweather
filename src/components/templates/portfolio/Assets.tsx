...
const Assets = ({currencySelect, setIsOpenAddModal, setIsOpenEditAssetsModal,userProfile,ptfData,isValidating,mutate }: Props) => {
  ...
  const handleOpen=()=>{
    if(userProfile?.portfolio && userProfile?.portfolio.length>=10){
      toast(router.locale=="ko" ? "자산이 10개를 초과할 수 없습니다. 자산 개수를 수정해주시길 바랍니다." : "You can't add more than 10 assets, please edit your assets first",{ hideProgressBar: true, autoClose: 2000, type: "error" })
      return
    }
    ...
    <main className="rounded-20 max-w-[650px] max-h-[822px] flex-1 overflow-hidden">
      ...
      <section className="bg-white pt-3 px-8  h-[680px] overflow-auto slim-scroll">
        {userProfile?.portfolio && userProfile?.portfolio.length>0 ? (
          ptfData?.map(({ name, ticker, quantity,price,krPrice,krName}:PortfolioData, i:number) => (
            <div key={i}>
              <article className="flex justify-between items-center" onClick={()=>router.push({pathname:`/detail/${ticker}`})}>
                <div className="w-[191px] flex">
                <Image loading="eager" unoptimized  width={50} height={50} quality={100} onError={handleImageError} src={`/images/logos/${ticker}.png` || ""} alt="" className="h-10 w-10 mr-3 "/>                      <div>
                    <h6 className="mb-1 w-[150px] truncate" title={router.locale=="ko" ? krName : name}>{router.locale=="ko" ? krName : name}</h6>
                ...
                <p className="text-[#111111] w-[191px] text-center">
                 {currencySelect===1 ? "￦" +" " + krPrice?.toLocaleString('en-US',{minimumFractionDigits: 0, maximumFractionDigits: 0}) : "$" + " " + price?.toLocaleString('en-US',{minimumFractionDigits: 2, maximumFractionDigits: 2})}
                </p>
              </article>
          ...
          <p className="  font-semibold text-md text-[#111111]">{t("noAssets")}</p>
          <p className="pb-[30px]  font-semibold text-md text-[#111111]">{t("noAssets2")}</p>

        <button           
        onClick={handleOpen}
        type="button" 
        className="w-[200px] h-[50px]   text-white bg-gradient-to-br from-purple-600 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-bold rounded-xl shadow-sm text-md px-5 py-2.5 text-center mr-2 mb-2">{t('assetAdd')}</button>
      ...
      <p className="pb-5 text-sm text-[#6B7280] text-end font-light"> {ptfData.length>0 && !isValidating ? udtDate : ""}</p>
