...
const Assets = ({session,interestedList}:Props) => {
  ...
  return (
    <main
      className="min-w-[1024px] relative bg-white text-center shadow-[0_0_12px_0_rgba(121,120,132,0.15)]"
    >
      ...
      <section className="pb-20 mb-7 max-w-[992px] mx-auto flex flex-wrap gap-12">
        ...
        {interestedList.map(({HR_ITEM_NM, ITEM_ENG_NM,ITEM_CD_DL,ITEM_KR_NM }:Asset, i:number) => (
          ...
          >
            ...
            <Image loading="eager" unoptimized  width={50} height={50} quality={100} onError={(event)=>handleImageError(event,HR_ITEM_NM)} src={`/images/logos/${ITEM_CD_DL}.png` || ""} alt="" className={`${selectCoin.some(asset=>asset['ticker']==ITEM_CD_DL) && "bg-light-gray-rgba"} 
       w-full h-full `}/>    
            ...
            <h5 className="text-xl text-[#111111] text-center w-full truncate" title={router.locale=="ko" ? ITEM_KR_NM : ITEM_ENG_NM}>
        ...
        {selectCoin.length>=2 &&!isModalOpen ? <div className="sticky bottom-[100px] w-full">
        <button
      ...
      </div> : ""}        {selectCoin.length==1 &&!isModalOpen ? <div className="sticky bottom-[100px] w-full">
        <button
...
<article className="h-fit border w-[415px] mx-auto bg-white py-12 rounded-20 top-1/2 left-1/2 z-20 ">
  ...
  <div className="flex items-center justify-center mb-5">
    <input onChange={()=>setIsApproved(prev => !prev)} id="link-checkbox" type="checkbox" value="" className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600"/>
    <label htmlFor="link-checkbox" className="ml-2 text-[10px] text-light text-gray-400 font-medium  dark:text-gray-300">{t('agreements')} <Link target="_blank" href="/policy" className="text-blue-600 dark:text-blue-500 hover:underline">{t('agreements2')}</Link></label>
</div>
  <button
