...
const Modal = ({
  ...
  return (
    <main className="fixed top-16 left-0 w-full h-full bg-white z-20 ">
      ...
      <div className={'mr-5 mt-5 justify-between flex text-md text-gray-400 text-center font-semibold'}>
          <div></div>
          ...
          {MOBILE_MENUS.map(({ id, title, path,koreanTitle }) => (
            <li key={id}>
              <Link
                ...
                onClick={()=>{onClose(); title==="Explore" ?  dispatch({ type: 'RESET_VALUE' }) : ""}}
              >
          ...
          <article className="flex-1 flex items-center">
            ...
            {session ? (
              ...
              <Link href={`/${router.locale}/login`} onClick={onClose}>
                <p className="text-gray-400 cursor-pointer">{router.locale=="ko" ? "로그인" : "Sign In"}</p>
              </Link>
            )}
...
>
            <p className={'mb-0.5'}>  {topRisk.length}</p>
            </div>}
            </div>
        </section>
      </div>
