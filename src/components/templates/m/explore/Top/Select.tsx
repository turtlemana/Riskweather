...
const Select = ({
  ...
  return (
    <article className="relative min-w-80  pl-5 rounded-20 mb-3">
      <label htmlFor={title} className="text-sm text-[#111111]   font-bold">{router.locale=="ko" ? koreanTitle :title}</label>
      <select id={title}
      value={defaultData}
        onTouchStart={() => setIsActive(true)}
        onBlur={()=>setIsActive(false)}
        disabled={title=="Location" ? tmpType=="Crypto" ? true : false : tmpType=="Crypto" || tmpType=="Index" ? true : false}
