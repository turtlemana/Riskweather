...
import { KeyboardEventHandler, useState,useEffect,useMemo,useCallback} from "react";
...
const handleKeyDown: KeyboardEventHandler<HTMLInputElement> = (event) => {
  if (event.key === "Enter") {
    event.preventDefault(); // 기본적인 Enter 키 동작 방지
  ...
  return (
    ...
    <div className="max-w-1320  mx-auto rounded-20 py-8 bg-white">
      <div className="flex justify-between">
        <h1 className="ml-8 text-[28px] text-[#111111] mb-10">{t("headerTitle")}</h1>
        <p className="mr-8 text-sm text-gray-400 mb-10">{assetList.length > 0 && !isValidating ? udtDate : ""}</p>
      </div>
      {/* Top */}
      <main className="max-w-1320 min-w-[1024px] overflow-auto">
        <section className="ml-8 border-b border-gray-100 pb-6 flex justify-between mr-12 lg:mr-8 ">
          <article className="flex gap-4 items-center">
            <h6 className="text-xs lg:text-sm text-[#111111] truncate">{t("typeTitle")}</h6>
              ...
              {TYPES.map(({ id, title, koreanTitle }) => (
                <li
                  ...
                  className={`truncate rounded-20 border px-3.5 py-1.5 text-center cursor-pointer max-h-[32px] hover:text-[#6B7280] hover:bg-[#F3F4F6] ${
                    state.type == title ? "border-[#E6F5FF] bg-[#E6F5FF]" : "border-gray-200"
                  }`}
                  onClick={() => {
          ...
          <article className="flex gap-4 items-center">
            <h6 className="text-xs lg:text-sm text-[#111111] truncate">{t("risklevelTitle")}</h6>
            <ul className="flex gap-[10px]">
              {RISKS.map(({ id, title, koreanTitle }) => (
                <li
                  ...
                  className={`text-sm truncate rounded-20 border px-3.5 py-1.5 text-center cursor-pointer max-h-[32px] hover:bg-[#F3F4F6] ${
                    state.riskLv === title ? "border-[#E6F5FF] bg-[#E6F5FF]" : "border-gray-200"
                  }`}
                  onClick={() => {
                ...
                >
                  <h6
                    className={`lg:text-sm text-[10px]  ${
                      state.riskLv === title ? "text-[#0198FF]" : "text-gray-500"
                    }`}
                  >
                    {router.locale === "ko" ? koreanTitle : title}
                  </h6>
          ...
          <Select defaultData={state.weather} data={WEATHERS} title={"Weather"} koreanTitle={"날씨"} selectHandler={weatherHandler} />

        </section>
        <section className=" ml-8 border-b border-gray-100 py-6 flex justify-between mr-12 ">
          <Select type={state.type} defaultData={state.loc} data={LOCATIONS} title={"Location"} koreanTitle={"국가"} selectHandler={locHandler} />
          <Select type={state.type} defaultData={state.exchg} data={EXCHAGNES} title={"Exchange"} koreanTitle={"거래소"} selectHandler={exchgHandler} />
          <Select type={state.type} defaultData={state.sect} data={SECTROS} title={"Sector"} koreanTitle={"분야"} selectHandler={sectHandler} />
        </section>
        <section className="ml-8 py-6 flex gap-10 ">
          ...
          <Select defaultData={state.limit} data={VIEWS} title={"Views"} koreanTitle={"표시"} selectHandler={rowHandler} />
          <Select defaultData={state.currency} data={CURRENCIES} title={"Currency"} koreanTitle={"통화"} selectHandler={currencyHandler} />
     ...
     <div className="border-t-[5px] border-gray-100" />
      <main className="mb-7">
        <div className="max-w-1320 w-full mx-auto bg-white rounded-20 overflow-hidden">
            ...
            <tbody>
              ...
              {assetList.map((asset, i) => (
                <Items
                  ...
                  price={state.currency === "KRW" ? asset['ADJ_CLOSE_KRW'] : asset['ADJ_CLOSE_USD']}
          ...
          <Pagination total={rowCount} page={state.page} setPage={(page) => dispatch({ type: 'SET_PAGE', payload: page })} views={state.limit} />
        </div>
      </main>
    </div>
  </main>
